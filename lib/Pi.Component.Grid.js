/*
	...
	Attivazione:
		data-pic = grid : {
					...
					startPage : <quale delle pagine è selezionata di default (index o valore)>
...
pi.component.register('grid',function(obj,settings){
	...
	var fnOnUpdate = function(){
		 ...
		 *		}
		 *	page : < pagina attuale >
		 *	pages : < numero totale di pagine >
		 *	count : < numero totale di righe >
		 *	}
		 */
		var data = JSON.parse(grid.find('.j-hidden-input').val()||null);
		if(data){
			grid.find('.j-sort>i').removeClass(iconAscending).removeClass(iconDescending).addClass(iconSort);
			grid.find('[data-name="' + data.sort.column + '"]>i').removeClass(iconSort).addClass( data.sort.direction === 'asc' ? iconAscending : iconDescending);
			var header = grid.find('.j-header');

			grid.find('.j-table').children().remove();
			fnGetGrid(cols,data.data,header).appendTo(grid.find('.j-table'));
			fnCreatePagesSelector(data.page,data.count);
		}
	};
	var MyGrid = fnGetGrid(cols,[],fnGetHeader());
	var MySelector ='';
	for(i = 0 ; i < cfg.pages.length; i++){
		MySelector += '<option value="' + cfg.pages[i] + '" ' + (cfg.startPage === i || cfg.startPage === cfg.pages[i] ? 'selected' : '' ) + '> ' + cfg.pages[i] + ' </option>';
	}
	grid.html('<div id="' + cfg.id + '">'
			...
			+'		<th><select class="small j-pages-selector"> ' + MySelector + ' </select></th>'
			+'	</tr></table>'
			+'</div>'
			+'<input type="hidden" name=":NULL" class="j-hidden-input">'
			+'</div>');
	MyGrid.appendTo(grid.find('.j-table'));
