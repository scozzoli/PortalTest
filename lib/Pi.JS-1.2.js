...
pi = (function(){ // membri privati
	...
	function _blur(){
		if(_win.active() || _loader.active() || _msg.active()){
			$('.pi-wrapper').addClass('blur');
		}else{
			$('.pi-wrapper').removeClass('blur');
		}
	}
	...
	var _win = {
		...
		resize: function(){
			...
			var maxHeight =  documentHeight - ($win.find(".header").outerHeight() || 0 ) - ( $win.find(".footer").outerHeight() || 0 ) - 20; // sfrido di 10 pixel sopra e sotto
			...
			$win.css('height',height + ( $win.find(".header").outerHeight() || 0 ) + ( $win.find(".footer").outerHeight() || 0 ));
			$win.css('width',width);
			var winTotalH = height + ($win.find(".header").outerHeight() || 0) + ($win.find(".footer").outerHeight() || 0);
	...
	function _getInputsByName(id){
		...
		var fnGetVal = function(call,idx,val){
			if(call[idx] != undefined){
				if(typeof call[idx] != 'object'){
					call[idx] = [ call[idx] ];
				}
				call[idx].push(val);
			}else{
			...
			if(inputs[i].name){
				if(inputs[i].name.indexOf(':') === 0){
					switch(inputs[i].name){
						case ':LINK:GRP' :
							listaLink = $.extend(null,_getInputsByName(inputs[i].value).call,listaLink);
							break;
						case ':LINK:ELEM' :
							var tmp = $('#'+inputs[i].value);
							if(tmp.length){
								fnGetVal(listaLink,tmp[0].name,tmp.val());
							}
						break;
						case ':URL' :
					...
					switch(inputs[i].type){
						...
						case "number" :
						case "time" :
							fnGetVal(lista.call,inputs[i].name,inputs[i].value);
							break;
						...
						case "radio" :
							...
							break;
						case "checkbox" :
							fnGetVal(lista.call,inputs[i].name,inputs[i].checked ? 1 : 0);
							break;
						case "file" :
		...
		for(var i in listaLink){
			if(lista.call[i] === undefined){
				lista.call[i] = listaLink[i];
			}
		}
		return lista;
	};
	...
	var _flexbox = {
		...
		parse: function(obj){
			var items = obj ? obj.find('f-grid') : $('f-grid');
			var newElem;
