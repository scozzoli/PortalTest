 ...
 */
 ...
 class PiRespose{
	...
	public function getLocalPath($iPath = ""){
		$sep = strpos($iPath,"/") === 0 ? "" : '/';
		$modPath = str_replace('../','',$this->postPi['module']);
		if(file_exists($this->opt['root'].'modules/'.$modPath.'/interface.php')){
			return $this->opt['root'].'modules/'.$modPath.$sep.$iPath;
		}else{
			$this->error("Errore nel recupero del path del modulo");
		}
	}
