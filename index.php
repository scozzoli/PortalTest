<?php
	...
	if(isset($_SESSION[MSID]["usr"])){
		$usr = $_SESSION[MSID]["usr"] == 'guest' ? ( $_GET["USR"] ?: 'guest') : $_SESSION[MSID]["usr"];
	}else{
		$usr = $_GET["USR"] ?: 'guest';
	}
	...
	if($usr == 'guest' && isset($_GET["USR"])){
		if(!$sd->initSession($_GET["USR"],true)){
			$sd->initSession($usr);
		}else{
			$usr = $_GET["USR"];
		}
	...
	/**
	 * Inserire qu� tutti i controlli EXTRA sugli utenti e le include aggiuntive:
	 * ATTENZIONE : se si modifica l'utente � necessario eseguire nuovamente "initSession( <Utente> )" e "select( <Modulo> [, <Gruppo>] )".
	 */
