...
```php
	$pr->info( 'system/test', array('saluto' => 'ciao', 'test'=>'ok') );	
...
Questa funzione accettava come parametro una stringa che corrispondeva all'id di un elemento html ed obbligava il client ad eseguire una scasione in tutti glie elementi al suo interno alla ricerca di *component* da elaborare. Questa operazione ora viene fatta in automatico ogni volta che si inietta html nella pagina attraverso le funzioni `addHtml` e `addWindow`.