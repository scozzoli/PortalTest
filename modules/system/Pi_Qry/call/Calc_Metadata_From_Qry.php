<?php
		...
		if($tmpDest != ''){			
			switch($v['type']){
				case 'data' : 
					...
					if($tmpDest === false) $pr->alert("Il campo {$v['des']} deve essere una data valida");
				break;
				case 'numeric' :
					$tmpDest = $pr->getNumber('i_'.$k);
					if($tmpDest === false) $pr->alert("Il campo {$v['des']} deve essere un numero valido");
				break;
				default:
	...
	if(count($res) == 0){
		$pr->alert("L'interrogazione non ha riportato risultati. Non &eacute; possibile recuperare la lista delle colonne");
	}
