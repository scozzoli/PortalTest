	...
	foreach($res as $idx => $row){
		...
		foreach($row as $k => $v){
			switch($qryConf['metadata'][strtolower($k)] ?: 'string'){
				case 'numeric' :
					$out .= xlsWriteNumber($idx+1,$j++,excelNull($v));
					break;
				case 'date' :
					$out .= xlsWriteNumber($idx+1,$j++,ExcelDate(excelNull($v),$qryConf['null']));
					break;
				case 'datecobol' :
					$out .= xlsWriteNumber($idx+1,$j++,ExcelDateCobol(excelNull($v),$qryConf['null']));
					break;
				case 'hidden' :
					break;
				default :
					$out .= xlsWriteLabel($idx+1,$j++,utf8_decode(excelNull($v)));
			}
		}
	}
