<?php
	...
	function ExcelDate($dtm,$null = ' '){
		$timeVal = is_object($dtm) ? $dtm->format('d/m/Y') : $dtm;
		$res = preg_match('/^(?:(?:31(\/|-|\.)(?:0?[13578]|1[02]))\1|(?:(?:29|30)(\/|-|\.)(?:0?[1,3-9]|1[0-2])\2))(?:(?:1[6-9]|[2-9]\d)?\d{2})$|^(?:29(\/|-|\.)0?2\3(?:(?:(?:1[6-9]|[2-9]\d)?(?:0[48]|[2468][048]|[13579][26])|(?:(?:16|[2468][048]|[3579][26])00))))$|^(?:0?[1-9]|1\d|2[0-8])(\/|-|\.)(?:(?:0?[1-9])|(?:1[0-2]))\4(?:(?:1[6-9]|[2-9]\d)?\d{2})$/',$timeVal);
		if($res == 1){
			if(strpos($res,'/') !== false){
				$d = explode('/',$out);
	...
	function ExcelDateObj($dtm,$null = ' '){
		if(is_string($dtm)){
			...
			if($res == 1){
				...
				$date = date_create((strlen($d[2]) == 2 ? '20'.$d[2] : $d[2]).'-'.$d[1].'-'.$d[0]);
				return $date;
			}else{
				return null;
			}
