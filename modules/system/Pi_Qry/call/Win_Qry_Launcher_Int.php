<?php
	...
	$buttonConfig = $pr->chkGrp('qry') ? '<button class="blue" onclick="pi.requestOnModal(\'config_qry\')"><i class="mdi mdi-settings"></i> Configura </button>' : '';
	$fill = Array();
	
	if(count($qryConf['inputs'])>0){
		$paramSave = '<input type="checkbox" name="saveparam" onclick="pi.silent().requestOnModal(\'config_qry\',\'Update_Flg_SaveParam\')" '.($usrConf['save'][$pr->post('qry')] ? 'checked' : '').'> Salva Paramtri ';
	...
	*/
	...
	$footer .= $qryConf['xls'] != 'disabled' ? '<button class="green" onclick="pi.downloadOnModal(\'win_launch_qry\',\'Qry_To_Sheet\'); pi.win.close();"><i class="mdi mdi-file-excel"></i> Esporta in excel </button>' : '';
	$footer .= $qryConf['html'] != 'disabled' ? '<button class="blue" onclick="pi.requestOnModal(\'win_launch_qry\',\'Qry_To_Html\')"><i class="mdi mdi-language-html5"></i> Visualizza </button>' : '';
